...
- **До** старта работы карточка `status: In progress` (и коммит). Implementer не ставит `Done`. Когда код сдан — parent ставит `In review` и запускает ревьюера; следующую `Not started` можно брать (ревьюер read-only). `Done` только после Approved. Needs fixes → снова `In progress`.
- **Blocked** — стоп pickup (вопрос человеку, нет ассета, открытая зависимость). Почему — в теле карточки. Не отдельный статус «есть вопросы?».
- Закрыл карточку текущего спринта (`Done`) **или** увёл её в `In review` — сразу берёт следующую по порядку спринта, без «следующая задача».
