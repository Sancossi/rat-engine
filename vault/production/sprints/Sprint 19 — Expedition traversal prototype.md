...
Пользователь 2026-09-09 явно разрешил начать [[feat-stride-game-studio-authoring]] с переносом остаточной ручной приёмки P1 на позже. A1 добавлен в текущий спринт: квалификация native проекта, перенос карт/Core, библиотека ресурсов, общий GUI/ZIP цикл. P1 не помечается выполненным; его остаточный manual gate больше не блокирует этот A1 по решению пользователя.
