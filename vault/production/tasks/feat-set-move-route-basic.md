...
Intent: NPC ходит по сетке без physics-ящиков. Формат и тик уже решены в [[research-set-move-route|research: Set Move Route]].
...
Origin: [[research-set-move-route|research: Set Move Route]]. Взято в [[Sprint 11 — Set Move Route]]. Не трогать `grey_yard.json` (это [[feat-grey-yard-move-route|feat: Grey yard move route]]).

Follow-up: [[chore-document-set-move-route-schema|chore: Document set_move_route in map schema]], [[feat-smooth-move-route-step|feat: Smooth move-route step]]

## Resolution

`CommandOp::SetMoveRoute` + `Command::route` (`RouteStep` Move/Wait/Turn) + `through`. Overlay `EventRuntime::event_overlay`. Interpreter: `command_index` на opcode, `route_index` + `wait_frames`. Foreground: игрок не блокирует шаг; Parallel ждёт; PlayerTouch только от шага игрока. Verify: `.\build\tests\rat_tests.exe "[route]"`. Play: cyan marker двигается после [[feat-grey-yard-move-route|feat: Grey yard move route]]. Review: Approved.

## Bugs found

none. Review Minor (нет строки в schema) → [[chore-document-set-move-route-schema|chore: Document set_move_route in map schema]].
