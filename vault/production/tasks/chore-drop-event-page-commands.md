...
Acceptance: dump/save не пишет `commands` (или пустой массив, schema: deprecated). Apply не компилирует graph → list для Play. `docs/archive/cpp/schemas/map-event.schema.md` обновлён. Карты `data/maps/` по возможности graph-only. Loader всё ещё принимает legacy commands через `commands_to_graph`.
