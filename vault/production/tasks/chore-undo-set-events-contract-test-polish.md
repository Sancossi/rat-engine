...
Intent: minor из re-review [[feat-edit-undo-redo|feat: Edit undo/redo command stack]] (Approved): кейс `set_events` clears them вызывает `set_events` уже после acknowledge. Нужна проверка wipe, пока `active_message` ещё «Hello».

Acceptance: `[events]` падает, если `set_events` при открытом диалоге оставляет message/lock.

Origin: [[feat-edit-undo-redo|feat: Edit undo/redo command stack]]
