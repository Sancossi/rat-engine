...
Intent: тонкий debug-порт у `rat-editor` (HTTP localhost или named pipe): snapshot, why-not, хвост лога, опционально скрин HWND — по образцу Godot MCP `run_project` / `get_debug_output` и Blender `get_viewport_screenshot`. Общий browser/Godot MCP окно GLFW не видит. Сначала файловый dump ([[feat-debug-snapshot-json|feat: Debug snapshot JSON]]); MCP — обёртка. Контекст: [[Agent Debug]].
