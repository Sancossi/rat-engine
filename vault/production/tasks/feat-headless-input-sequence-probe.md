...
Intent: прогон N фиксированных шагов (120 Hz) с заранее заданными `PlayerFrameInput` без GLFW — репро «прыгнул и провалился» для агента. Зависит от кадра ввода ([[feat-input-action-mapping|feat: Input action mapping]]) или текущего `PlayerFrameInput`. На каждом шаге опционально snapshot. Контекст: [[Agent Debug]], [[ADR-010 Catch2 unit and headless mechanics tests]].
...
Follow-up: [[chore-input-sequence-probe-review-polish|chore: input sequence probe review polish]]
