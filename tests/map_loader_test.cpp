...
#error RAT_TEST_DATA_DIR must be defined for map file tests
  ...
  REQUIRE(result.map.schema_version == 5);
  ...
  REQUIRE(from_string.map.schema_version == 5);
  ...
  REQUIRE(from_serialized.map.schema_version == 5);
...
#error RAT_TEST_DATA_DIR must be defined for map file tests
  ...
  bool bridge_slab_65 = false;
  int bridge_span_slabs = 0;
  for (const rat::FloorSlabDef& slab : map.floor_slabs) {
    if (slab.tile.z != 5 || slab.top_y != Catch::Approx(2.0f) || slab.thickness != Catch::Approx(0.25f)) {
      continue;
    }
    if (slab.tile.x >= 6 && slab.tile.x <= 8) {
      ++bridge_span_slabs;
    }
    if (slab.tile.x == 6) {
      bridge_slab_65 = true;
  ...
  REQUIRE(bridge_slab_65);
  REQUIRE(bridge_span_slabs == 3);

  bool low_approach = false;
  bool high_approach = false;
  for (const rat::OccupancyCell& cell : map.occupancy) {
    if (cell.kind != rat::OccupancyKind::Ramp) {
      continue;
    }
    if (cell.x == 4 && cell.y == 0 && cell.z == 5 && cell.yaw == rat::RampDirection::East) {
      low_approach = true;
    }
    if (cell.x == 5 && cell.y == 1 && cell.z == 5 && cell.yaw == rat::RampDirection::East) {
      high_approach = true;
    }
  }
  REQUIRE(low_approach);
  REQUIRE(high_approach);
