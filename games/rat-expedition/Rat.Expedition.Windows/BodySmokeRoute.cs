...
internal sealed class BodySmokeRoute
{
    ...
    public TraversalInput Next(TraversalSnapshot s)
    {
        ...
        bool Near(float x,float z) => Vector2.Distance(new(s.Position.X,s.Position.Z),new(x,z))<.05f;
        switch(phase)
        {
            case 0:
                if(Near(-4,3)) phase=1;
                return Towards(-4,3);
            case 1:
                if(s.Position.Z<=1.901f) { Capture="standing-blocked";phase=2;return new(Vector2.Zero); }
            ...
            case 4:
                if(s.Stance==BodyStance.Standing && s.Position.Z<.95f) { Capture="clear-standing";phase=5; }
                return Towards(-4,.7f);
            case 5:
                if(Near(-4.4f,-.95f)) { phase=6;return new(Vector2.Zero,false,true); }
                return Towards(-4.4f,-.95f);
            case 6:
                if(!upCaptured && s.Mode==TraversalMode.Climbing && s.Position.Y>.75f) {Capture="climb-up";upCaptured=true;}
                if(s.Mode==TraversalMode.Grounded && s.Position.Y==1.6f) {Capture="upper-exit";phase=7;return new(Vector2.Zero,false,true);}
            ...
            case 10:
                if(!downCaptured && s.Mode==TraversalMode.Climbing && s.Position.Y<.9f) {Capture="climb-down";downCaptured=true;}
