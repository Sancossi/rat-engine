...
Очередь реализации: Sprint 7 (vault `production/sprints/Sprint 7 — Engine architecture.md`).

## Контекст

Roadmap основан на реализации из `feat/sprint-4` (Sprint 5–6 уже закрыты:
terrain cubes, edge walls, viewport edit, undo высот/рёбер). В `main` на момент
составления документа был только стартовый README; актуальный код движка всё
ещё нужно довести до основной ветки.
...
Terrain cubes, edge barriers, greybox walls и terrain/barrier undo закрыты в
Sprint 5–6. Оставшийся порядок — Sprint 7:

1. CI Windows/Linux и headless smoke `grey_yard`.
2. Граф зависимостей: `rat_core` без GLFW/ImGui/bgfx.
3. `SimulationSession`.
4. Replay и state checksum.
5. `MapDocument` и compilation в `RuntimeMap`.
6. Декомпозиция `EditorApp`.
7. Asset registry и первая загрузочная вертикаль.
8. Entity model ADR.
9. Render packets и instrumentation.
10. Platform/input/audio adapters.
11. Метрики кадра (оптимизации — только после замера).
