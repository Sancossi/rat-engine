[CmdletBinding()]
param([string]$CheckoutPath,[string]$SolutionPath,[string]$QualificationResult,[ValidateSet('session','resources','readiness-failure','creation-failure','creation-reopen')][string]$QualificationMode='session',[switch]$PassThru,[hashtable]$LaunchOwnership,[ValidateRange(1,60)][int]$ReadinessTimeoutSeconds=60)
