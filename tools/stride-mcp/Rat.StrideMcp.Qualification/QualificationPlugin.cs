...
public sealed class QualificationPlugin:AssetsPlugin
{
    ...
    public override void InitializeSession(SessionViewModel session){_=StartupHook.Mode switch{"actions" or "actions-reopen"=>AssetActionsQualification.Run(session),"resources"=>ResourceQualification.Run(session),"creation-failure" or "creation-reopen"=>CreationFailureQualification.Run(session),_=>Run(session)};}
